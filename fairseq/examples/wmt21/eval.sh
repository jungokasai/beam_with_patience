...
if [ ! -d "${TMP_DIR}/wmt20-news-systems" ]; then
...
INPUT_FILE=$TMP_DIR/wmt20-news-systems/txt/sources/newstest2020.${SRC}-${TGT}.src.${SRC}
# REF_FILE=$TMP_DIR/wmt21-news-systems/txt/references/newstest2021.${SRC}-${TGT}.ref.A.${TGT}
REF_FILE=$TMP_DIR/wmt20-news-systems/txt/references/newstest2020.${SRC}-${TGT}.ref.${TGT}

# Translate
cat ${INPUT_FILE} | sed "s/^/${DOMAIN_TAG} /" | perl $REPLACE_UNICODE_PUNCT | perl $NORM_PUNCT -l ${SRC} | python $PATH_TO_FAIRSEQ_PY/fairseq_cli/interactive.py  $MODEL_DIR \
  ...
  --lenpen 1.0 \
  --patience-factor $P \
  --fp16  > $TMP_DIR/${SRC}-${TGT}_${P}.gen_log

cat $TMP_DIR/${SRC}-${TGT}_${P}.gen_log | grep -P "^D-" | cut -f3 > $TMP_DIR/${SRC}-${TGT}_${P}.hyp

# Calculate BLEU score
sacrebleu -l $SRC-$TGT $REF_FILE < $TMP_DIR/${SRC}-${TGT}_${P}.hyp
